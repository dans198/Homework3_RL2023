...
Eigen::Matrix<double,6,7> KDLRobot::getJacobian()
{
    /*compute the analitic jacobian*/
    return s_J_ee_.data;
}
KDL::Frame KDLRobot::getCartesianPose() 
{
    /*get the cartesian pose of the ee*/
    KDL::Frame p_out;
    fkSol_->JntToCart(jntArray_, p_out, -1);
    return p_out; 
}
KDL::Twist KDLRobot::getCartesianVelocity()
{
    /*get the cartesian velocity of the ee*/
    KDL::FrameVel out;
    KDL::JntArrayVel jointVelocities(jntArray_,jntVel_); //KDL::JntArrayVel jointVelocities(chain_.getNrOfJoints());
    //KDL::diff(jntArray_,jointVelocities.qdot);
    fkVelSol_->JntToCart(jointVelocities, out, -1);
    return out.GetTwist();
}
Eigen::Matrix<double,6,1> KDLRobot::getJacDotqDot()
{
    /*compute the (Jacobian_dot * q_dot) term needed for the y*/
    KDL::Twist jac_dot_q_dot;
    KDL::JntArrayVel jointVelocities(jntArray_,jntVel_);
    jntJacDotSol_->JntToJacDot(jointVelocities, jac_dot_q_dot, -1);
    return toEigen(jac_dot_q_dot);
}

KDL::JntArray KDLRobot::getDesVel(KDL::Twist des_cart_vel,KDL::Jacobian J_cam) {
    /*compute desired velocity*/
    KDL::JntArray out;
    out.resize(chain_.getNrOfJoints());
    Eigen::Matrix<double,6,7> J_ee = toEigen(J_cam);
    Eigen::Matrix<double,7,6> Jpinv = pseudoinverse(J_ee);
    Eigen::Matrix<double,6,1> des_v = toEigen(des_cart_vel);
    Eigen::VectorXd dqd = Jpinv * des_v;
    out.data = dqd;
