...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            frame_offset.p=cam_T_object.p -KDL::Vector(0,0,0.4);
            ...
            /*-------2b----------*/
            /*
            Eigen::Matrix<double,3,1> c_Pob=toEigen(cam_T_object.p);
            Eigen::Matrix<double,3,1> s= c_Pob/c_Pob.norm();
            Eigen::Matrix<double,3,3> R_c = toEigen(robot.getEEFrame().M);
            Eigen::Matrix<double,3,3> L_block = (-1/c_Pob.norm())*(Eigen::Matrix<double,3,3>::Identity() - s*s.transpose());
            Eigen::Matrix<double,3,6> L = Eigen::Matrix<double,3,6>::Zero();
            Eigen::Matrix<double,6,6> R_C = Eigen::Matrix<double,6,6>::Zero();
            R_C.block(0,0,3,3) = R_c;
            R_C.block(3,3,3,3) = R_c;
            ...
            //q_dot equation
            int k = 6;
            dqd.data = k*LJ_pinv*Eigen::Vector3d(0,0,1)+10*N*(qdi - toEigen(jnt_pos));
            */
