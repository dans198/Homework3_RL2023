...
int main(int argc, char **argv)
{
    ...
    double trajRadius = 0.1; //AGGIUNTO
    KDLPlanner planner_cubic(traj_duration, init_position, trajRadius); //AGGIUNTO

    // Retrieve the first trajectory point
    trajectory_point p = planner_cubic.compute_trajectory(t);   //MODIFICATO

    // Gains
    double Kp = 25, Kd = sqrt(Kp);
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            {
                p = planner_cubic.compute_trajectory(0.0);
            }
            else if(t > init_time_slot && t <= traj_duration + init_time_slot)
            {
                p = planner_cubic.compute_trajectory(t-init_time_slot);
