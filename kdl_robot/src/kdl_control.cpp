...
Eigen::VectorXd KDLController::idCntr(KDL::JntArray &_qd, KDL::JntArray &_dqd, KDL::JntArray &_ddqd, double _Kp, double _Kd){
    // read current state
...
Eigen::VectorXd KDLController::idCntr(KDL::Frame &_desPos,KDL::Twist &_desVel, KDL::Twist &_desAcc, double _Kpp, double _Kpo, 
double _Kdp, double _Kdo)
{
    ...
    Eigen::Matrix<double,7,7> M = robot_->getJsim();    /*Jsim := Joint Space Inertial Matrix*/
    ...
    x_tilde << e_p, e_o;
    dot_x_tilde << dot_e_p, dot_e_o; //dot_x_tilde << dot_e_p, -omega_e;//dot_e_o;
...
Eigen::VectorXd KDLController::idCntr(KDL::Frame &_desPos,KDL::Twist &_desVel, KDL::Twist &_desAcc, double _Kpp, double _Kpo, 
double _Kdp, double _Kdo, Eigen::Matrix<double,3,1> e_o, Eigen::Matrix<double,3,1> e_o_w)
{
    // calculate gain matrices
    Eigen::Matrix<double,6,6> Kp, Kd;
    Kp.block(0,0,3,3) = _Kpp*Eigen::Matrix3d::Identity();
...
//Eigen::Matrix<double,3,3> KDLController::shCntr(Eigen::Vector3d lin_acc)
//{
    /*compute the shared controller*/
